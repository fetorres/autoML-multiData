...
import org.deidentifier.arx.gui.view.impl.menu.DialogAutoMLInfo;
...
public class MainWindow implements IView {
	...
	/** Output Path, added by Yunhui*/
	private String outPath;
	/** Output specification files, added by Yunhui*/
	private String outSpec;
	/** Number of saved privacy configurations, added by Yunhui*/
	private int savedCnt;
	/** Whether AutoML has provided valid outPath and input file*/
	private boolean autoML = false; //TODO add functions to set this to TRUE
	...
	public MainWindow(Display display, Monitor monitor) {
		...
		// Create the subviews
		Composite item1 = root.createItem(Resources.getMessage("MainWindow.2"), //$NON-NLS-1$
		...
		Composite item2 = root.createItem(Resources.getMessage("MainWindow.3"), //$NON-NLS-1$
		...
		Composite item3 = root.createItem(Resources.getMessage("MainWindow.1"), //$NON-NLS-1$
		...
		Composite item4 = root.createItem(Resources.getMessage("MainWindow.4"), //$NON-NLS-1$
		...
		root.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				controller.update(
						new ModelEvent(this, ModelPart.SELECTED_PERSPECTIVE, controller.getModel().getPerspective()));
			}
		});
	...
	public void setOutPath(String outPath) {
		...
		this.outSpec = outPath + "output.txt";
		File f;
		try {
			f = new File(outSpec);
			if (!f.exists()) {
				f.createNewFile(); // create a file with the name outputs.txt
			} else {
				f.delete();
				f.createNewFile(); // if the output.txt file already exits, delete the old one and create a new one
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/**
	 * Added by Yunhui
	 * Get the number of saved anonymized files
	 * @return number of saved anonymized files
	 */
	...
	/**
	 * Added by Yunhui
	 * increase the counter for saved anonymized files by one
	 */
	...
	/**
	 * Return the path to the output specification file "output.txt"
	 * @return the output specification file
	 */
	...
	public void onShow(final Runnable runnable) {
		...
		// Using a paint listener is a hack to reliably determine when the shell
		// is visible
	...
	/**
	 * Added by Yunhui
	 * Shows an AutoML Dialog
	 */
	public void showAutoMLDialog() {
		final DialogAutoMLInfo dialog = new DialogAutoMLInfo(shell, controller);
		dialog.create();
		dialog.open();
	}
	...
	public Charset showCharsetInputDialog(final Shell shell) {
		...
		final DialogComboSelection dlg = new DialogComboSelection(shell, Resources.getMessage("MainWindow.19"), //$NON-NLS-1$
				...
				charsets.toArray(new String[] {}), Charsets.getNameOfDefaultCharset(), validator);
	 ...
	 * @param handle
	 * @param column
	 * @return A pair containing the string to be found and the string with
	 *         which it is to be replaced, <code>null</code> if cancel was
	 *         pressed.
	 */
	 ...
	 * @param text
	 * @param preselected
	 *            Preselected format string, can be null
	 * @param locale
	 *            The current locale
	 * @param description
	 * @param values
	...
	public String showFormatInputDialog(final Shell shell, final String header, final String text,
			final String preselected, final Locale locale, final DataTypeDescription<?> description,
		...
		// Open dialog
		final DialogComboSelection dlg = new DialogComboSelection(shell, header, text, formats.toArray(new String[] {}),
				initial, validator);
	 ...
	 * @return
	 */
	public List<String> showMultiSelectionDialog(Shell shell, String title, String text, List<String> elements,
	 ...
	 * @return
	 */
	public String[] showOrderValuesDialog(final Shell shell, final String header, final String text,
	 ...
	 * 
	 * @param type
	 * @return A pair containing the bottom value + inclusive and the top value
	 *         + inclusive. Either bottom or top may be <code>null</code> if
	 *         they have not been defined. The overall pair may be
	...
	private MainMenuItem getEditMenu() {
		...
		items.add(new MainMenuItem(Resources.getMessage("MainMenu.21"), //$NON-NLS-1$
			...
			public void action(Controller controller) {
				controller.actionMenuEditAnonymize(false);
			}
		...
		items.add(new MainMenuItem(Resources.getMessage("MainMenu.40"), //$NON-NLS-1$
				...
				true) {
			public void action(Controller controller) {
				controller.actionMenuEditAnonymize(true);
			}
		...
		items.add(new MainMenuItem(Resources.getMessage("MainMenu.39"), //$NON-NLS-1$
				...
				true) {
			public void action(Controller controller) {
				controller.actionMenuEditReset();
			}
		...
		items.add(new MainMenuItem(Resources.getMessage("MainMenu.41"), //$NON-NLS-1$
			...
			public boolean isEnabled(Model model) {
				return model != null && model.getSelectedAttribute() != null
						&& model.getPerspective() == Perspective.CONFIGURATION;
			}
		});
	 ...
	 * Changed by Yunhui to add new item: "AutoML instructions:"
	 * 
	 * @return
	 */
	private MainMenuItem getHelpMenu() {
		...
		// added by Yunhui
		// add button to show the AutoML instructions
		items.add(new MainMenuSeparator());

		items.add(new MainMenuItem(Resources.getMessage("AutoML.11"), //$NON-NLS-1$
				controller.getResources().getManagedImage("information.png"), //$NON-NLS-1$
				false) {
			public void action(Controller controller) {
				controller.actionMenuHelpAutoML();
			}
	...
	private MainMenuItem getMenuFile() {
		...
		// added by Yunhui, add new button "Export to AutoML"
		items.add(new MainMenuItem(Resources.getMessage("AutoML.8"), //$NON-NLS-1$
				controller.getResources().getManagedImage("file_export_data.png"), //$NON-NLS-1$
				true) {
			public void action(Controller controller) {
				controller.actionMenuFileExportDataToAutoML(outPath);
			}

			public boolean isEnabled(Model model) {
				return model != null && model.getOutput() != null && model.getPerspective() == Perspective.ANALYSIS && !outPath.equals("");
			}
		});
	...
	private MainMenuItem getViewMenu() {
		...
		items.add(new MainMenuItem(Resources.getMessage("MainWindow.2"), //$NON-NLS-1$
			...
			public void action(Controller controller) {
				root.setSelection(0);
				controller.getModel().setPerspective(Perspective.CONFIGURATION);
				controller.update(new ModelEvent(controller, ModelPart.SELECTED_PERSPECTIVE,
						controller.getModel().getPerspective()));
			}
		...
		items.add(new MainMenuItem(Resources.getMessage("MainWindow.3"), //$NON-NLS-1$
			...
			public boolean isEnabled(Model model) {
				return model != null && model.getPerspective() != Perspective.EXPLORATION;
			}
		});
		...
		items.add(new MainMenuItem(Resources.getMessage("MainWindow.1"), //$NON-NLS-1$
			...
			public boolean isEnabled(Model model) {
				return model != null && model.getPerspective() != Perspective.ANALYSIS;
			}
		});
		...
		items.add(new MainMenuItem(Resources.getMessage("MainWindow.4"), //$NON-NLS-1$
			...
			public boolean isEnabled(Model model) {
				return model != null && model.getPerspective() != Perspective.RISK;
			}
		});
		...
		items.add(new MainMenuItem(Resources.getMessage("MainMenu.34") + " " + Resources.getMessage("ExploreView.0"), //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			...
			public boolean isEnabled(Model model) {
				return model != null && model.getPerspective() == Perspective.EXPLORATION && model.getResult() != null
