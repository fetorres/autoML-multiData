...
public class EditorCriterionRiskBased extends EditorCriterion<ModelRiskBasedCriterion>{
    ...
    protected Composite build(Composite parent) {
        ...
        // added by Yunhui, add explnations for privacy model
    	String modelString = "";
    	if (isPopulation) {
    		modelString = Resources.getMessage("AutoML.13e2");
    	} else if (model.getVariant() == ModelRiskBasedCriterion.VARIANT_AVERAGE_RISK) {
    		modelString = Resources.getMessage("AutoML.13e1");
    	} else if (model.getVariant() == ModelRiskBasedCriterion.VARIANT_SAMPLE_UNIQUES) {
    		modelString = Resources.getMessage("AutoML.13e3");
    	}
    	
        final Composite parentGroup = new Composite(parent, SWT.NONE);
    	parentGroup.setLayoutData(SWTUtil.createFillHorizontallyGridData());
    	final GridLayout compositeGridLayout = new GridLayout();
    	compositeGridLayout.numColumns = 1;
    	parentGroup.setLayout(compositeGridLayout);
        createInstructions(parentGroup, modelString);  

        // Create input group
        final Composite group = new Composite(parentGroup, SWT.NONE); // Yunhui: changed from parent to parentGroup
